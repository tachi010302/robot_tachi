...
class Transforms:
    def __init__(self):
        ...
        rospy.init_node('odom', disable_signals = True)
...
if __name__ == '__main__':
    ...
    #CODIGO ORIGIAL diff_tf  YA QUE SE JUNTARON Y ES EL QUE ESTA OPERANDO ACTUALMENTE 
...
class DiffTf:
    ...
    def __init__(self):
        ...
        self.ticks_meter = float(rospy.get_param('ticks_meter', 6150))  # The number of wheel encoder ticks per meter of travel 3965
        ...
        self.x = 0                 # posision en el plano xy 
        self.y = 0
        self.th = 0
        self.dx = 0                 # velocidad de rotacion en x 
        self.dr = 0
    ...
    def update(self):
        ...
        if now > self.t_next:
            ...
            if (d != 0):  
                # calcula la distancia viajada en x y en y 
                x = cos( th ) * d
                y = -sin( th ) * d
                # calcula la posicion final del robot 
                self.x = self.x + ( cos( self.th ) * x - sin( self.th ) * y )
                self.y = self.y + ( sin( self.th ) * x + cos( self.th ) * y )
            if( th != 0):
                self.th = self.th + th
                
            # publica la informacion de la odometria 
